...
const questions = [
 ...
 { text: "Could your child put common items such as fruit, animals, or vehicles into the correct category?", domain: "receptive", reverse: false },
 { text: "Does your child correctly answer questions beginning with who, what, or where?", domain: "receptive", reverse: false },
...
const score = Math.round((rawScore / 72) * 80);
   ...
   const { data: previousRecords, error: fetchError } = await supabase
     .from("records")
     .select("score, created_at, assessment_number")
     .eq("family_code", code)
   ...
   const previousCount = previousRecords ? previousRecords.length : 0;
   const assessmentNumber = previousCount + 1;
   const previousScore = previousRecords && previousRecords.length > 0 ? previousRecords[0].score : null;
 ...
 if (!accepted) {
   return (
     <div style={{ padding: 40, maxWidth: 800 }}>
       ...
       <p>
         This tool is a developmental screening and parent guidance system.
       ...
       <p>
         If this is your first time, choose “First Time.” The system will create a Family Code for you.
         Please save it for future re-screening.
       </p>

       <p>
         If you have completed this screening before, choose “I Already Have a Code” and enter your existing Family Code.
       </p>

       <button
         onClick={() => setHasCode(false)}
         style={{ marginRight: 10, padding: 10, background: hasCode ? "#eee" : "#ccc" }}
       ...
       <button
         ...
         style={{ padding: 10, background: hasCode ? "#ccc" : "#eee" }}
    ...
    For children aged 4–6, persistent concerns in these areas may be more clinically meaningful. This result may suggest a stronger need for close monitoring and consideration of professional consultation, especially if concerns persist.
       ...
       {result.previous_score !== null ? (
         <div>
           ...
           <p><b>Previous Score:</b> {result.previous_score}</p>
           <p><b>Current Score:</b> {result.score}</p>
           <p><b>Change:</b> {result.score_change > 0 ? "+" : ""}{result.score_change}</p>
         </div>
       ) : (
         <p>This is the first screening recorded for this Family Code.</p>
       )}
       ...
       {Object.keys(result.domains).map((d) =>
         result.domains[d] !== "Strong" ? (
           <div key={d}>
             <b>{d}</b>
             <ul>
               {strategies[d].map((s: string, i: number) => (
                 <li key={i}>{s}</li>
               ))}
             </ul>
           </div>
       ...
       <p><b>Please save your Family Code for future re-screening:</b> {result.family_code}</p>
     </div>
   );
 }
